defmodule Shop.PromotionsTest do
  ...
  describe "promotions" do
    ...
    test "create_promotion/1 with valid data creates a promotion" do
      valid_attrs = %{code: "some code", name: "some name", expires_at: ~U[2026-02-26 18:00:00Z]}
      ...
      assert promotion.expires_at == ~U[2026-02-26 18:00:00Z]
    end
    ...
    test "update_promotion/2 with valid data updates the promotion" do
      promotion = promotion_fixture()
      update_attrs = %{code: "some updated code", name: "some updated name", expires_at: ~U[2026-02-27 18:00:00Z]}
      ...
      assert promotion.expires_at == ~U[2026-02-27 18:00:00Z]
    end
  ...
  describe "promotions" do
    ...
    test "create_promotion/1 with valid data creates a promotion" do
      valid_attrs = %{code: "some code", name: "some name", expires_at: ~U[2026-02-27 10:42:00Z]}

      assert {:ok, %Promotion{} = promotion} = Promotions.create_promotion(valid_attrs)
      assert promotion.code == "some code"
      assert promotion.name == "some name"
      assert promotion.expires_at == ~U[2026-02-27 10:42:00Z]
    end
    ...
    test "update_promotion/2 with valid data updates the promotion" do
      promotion = promotion_fixture()
      update_attrs = %{code: "some updated code", name: "some updated name", expires_at: ~U[2026-02-28 10:42:00Z]}

      assert {:ok, %Promotion{} = promotion} = Promotions.update_promotion(promotion, update_attrs)
      assert promotion.code == "some updated code"
      assert promotion.name == "some updated name"
      assert promotion.expires_at == ~U[2026-02-28 10:42:00Z]
    end
